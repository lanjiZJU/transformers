...
def main():
    ...
    if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        model_to_save = ner_model.module if hasattr(ner_model, "module") else ner_model  # Take care of distributed/parallel training
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            ner_model_.load_state_dict(state_dict)
            ner_model_.to(args.device)
            result, _ = evaluate(args, ner_model_, tokenizer, labels, pad_token_label_id, mode="dev", prefix=global_step)
            if global_step:
    ...
    if args.do_predict and args.local_rank in [-1, 0]:
        ...
        checkpoint = os.path.join(args.output_dir, WEIGHTS_NAME)
        state_dict = torch.load(checkpoint)
        ner_model_.load_state_dict(state_dict)
        ner_model_.to(args.device)

        result, predictions = evaluate(args, ner_model_, tokenizer, labels, pad_token_label_id, mode="test")
        # Save results
